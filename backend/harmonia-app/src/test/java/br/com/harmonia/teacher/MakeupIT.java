...
class MakeupIT {
    ...
    @Test
    void teacher_cannotCreateSecondMakeupForSameOriginalLesson() throws Exception {
        String admin = login("admin", "Admin@123");
        ...
        String t = login("teacherMk2", "Teach@1234");
        String today = LocalDate.now().toString();
        ...
        mvc.perform(post("/teacher/lessons/" + originalLesson + "/makeup").header("Authorization", "Bearer " + t)
            ...
            .andExpect(jsonPath("$.code", org.hamcrest.Matchers.is("INVALID_MAKEUP_LINK")));
    }
}
