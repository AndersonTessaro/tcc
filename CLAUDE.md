...
13. ✅ **Revisão de criação (aula/horário/matrícula)** — `GET /teacher/enrollments`, `GET /admin/{students,teachers,instruments}` (seletores no lugar de UUID digitado); respostas via DTOs em `presentation/response` (sem entidade JPA); frequência recusa aula `CANCELED`/futura; horário recorrente valida contra aulas futuras e na reativação; matrícula duplicada ativa → 409 `DUPLICATE_RESOURCE`; id inexistente → 404. Aula com data futura nasce `SCHEDULED`; reposição aceita original `DONE` ou `CANCELED`. Clientes lançam `ApiError` (status+code) e `apiErrorMessage` traduz p/ PT. Mobile: telas Horários fixos (Mais → Horários) e Reposição (Agenda → Repor).

> Build/test backend: `cd backend && docker compose up -d && ./mvnw verify` (reactor builda `lesson-core` → `harmonia-app`; 41 unit no core, 14 unit + 45 IT no app (ST inclusos no `verify`)). Rodar app: `./mvnw -pl harmonia-app -am package -DskipTests && java -jar harmonia-app/target/harmonia-app-0.0.1-SNAPSHOT.jar` (`-am spring-boot:run` quebra: o plugin roda também no pom pai e falha com `ClassNotFoundException: SpringApplication`). Roteiro Postman: `npx newman run postman/Harmonia-lesson-core.postman_collection.json`. Admin: `admin`/`Admin@123`.
