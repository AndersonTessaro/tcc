...
> Build/test backend: `cd backend && docker compose up -d && ./mvnw verify` (reactor builda `lesson-core` → `harmonia-app`; 32 unit no core, 14 unit + 32 IT no app, + 2 ST no perfil `system-tests`). Rodar app: `./mvnw -pl harmonia-app -am spring-boot:run`. Admin: `admin`/`Admin@123`.
