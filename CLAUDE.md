...
> Admin seed: usuário `admin` / senha `Admin@123` (trocar em prod). Postgres dev: `docker compose up -d` na raiz do repo (compose em `docker-compose.yml`; `spring-boot-docker-compose` sobe sozinho ao rodar pelo IDE/`spring-boot:run`).
...
- `docs/lesson-core-framework.md` — arquitetura, regras e integração do framework de aulas.
...
13. ✅ **Revisão de criação (aula/horário/matrícula)** — `GET /teacher/enrollments`, `GET /admin/{students,teachers,instruments}` (seletores no lugar de UUID digitado); respostas via DTOs em `presentation/response` (sem entidade JPA); frequência recusa aula `CANCELED`/futura; horário recorrente valida contra aulas futuras e na reativação; matrícula duplicada ativa → 409 `DUPLICATE_RESOURCE`; id inexistente → 404. Aula com data futura nasce `SCHEDULED`; reposição aceita original `DONE` ou `CANCELED`. Clientes lançam `ApiError` (status+code) e `apiErrorMessage` traduz p/ PT. Mobile: telas Horários fixos (Mais → Horários) e Reposição (Agenda → Repor). Matrícula exige que o professor ensine o instrumento (`teacher_instrument`; `POST /admin/teachers` aceita `instrumentIds`, `PUT /admin/teachers/{id}/instruments` edita). ADMIN faz bypass de ownership (RN13) nas ações sobre recurso; rotas "minhas" sem perfil → 403 `PROFILE_REQUIRED`.

> Build/test backend: `docker compose up -d && cd backend && ./mvnw verify` (reactor builda `lesson-core` → `harmonia-app`; 41 unit no core, 14 unit + 48 IT no app (ST inclusos no `verify`)). Rodar app: `./mvnw -pl harmonia-app -am package -DskipTests && java -jar harmonia-app/target/harmonia-app-0.0.1-SNAPSHOT.jar` (`-am spring-boot:run` quebra: o plugin roda também no pom pai e falha com `ClassNotFoundException: SpringApplication`). Roteiro Postman: `npx newman run postman/Harmonia-lesson-core.postman_collection.json`. Admin: `admin`/`Admin@123`.
...
> Docker backend: `backend/Dockerfile` (multi-stage, JRE 25 alpine, usuário sem privilégio). Stack completa: `docker compose --profile app up --build` (na raiz) (o `up -d` puro segue só Postgres). Chaves RSA: a imagem carrega o par do classpath; dá p/ sobrepor montando em `/app/certs` ou setando `APP_RSA_PRIVATE_KEY`+`APP_RSA_PUBLIC_KEY` (as duas, ou falha), e `HARMONIA_DEV_KEYS=true` gera par descartável.
