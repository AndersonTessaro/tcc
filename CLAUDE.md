...
> Build/test backend: `cd backend && docker compose up -d && ./mvnw verify` (reactor builda `lesson-core` → `harmonia-app`; 41 unit no core, 14 unit + 45 IT no app (ST inclusos no `verify`)). Rodar app: `./mvnw -pl harmonia-app -am spring-boot:run`. Admin: `admin`/`Admin@123`.
